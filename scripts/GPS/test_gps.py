#!/usr/bin/env python3
"""
Mapviz click GPS planner with optional localized mode.
...
- In localized + anchor mode, a local tangent approximation is used:
  the clicked GOAL is geographic, and the robot is treated as the local origin for route anchoring.
        ...
        if self.localized:
            try:
                if self.use_fromll:
                    ...
                    # In anchor mode we still need geographic endpoints for OSMnx.
                    # Best available approximation without /toLL:
                    # use goal UTM zone and treat robot as local origin; route graph start is approximated
                    # by a very small offset around goal if exact robot GPS is unavailable.
                    self.get_logger().warning(
...
def main():
    ...
    parser.add_argument("--spacing", type=float, default=20.0, help="Densify spacing in meters (smaller => more points)")
